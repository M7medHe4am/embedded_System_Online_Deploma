...
uint32_t (*USART_getCLCK[])(void) = {MCAL_RCC_getAPB2CLKFreq ,MCAL_RCC_getAPB1CLKFreq ,MCAL_RCC_getAPB1CLKFreq } ;
 ...
 * @retval 			-none
 * Note				-MUST initialize USART first
 * 					-When transmitting with the parity enabled (PCE bit set to 1 in the Uمين SART_CR1 register),
 * 					the value written in the MSB (bit 7 or bit 8 depending on the data length) has no effect
 * 					because it is replaced by the parity
 */
